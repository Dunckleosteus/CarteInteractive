...
ancien_nom_vers_nouveau_nom = {
    ...
    'Dans quelle ville a lieu ton stage ?': 'Ville','Quelle est ton entreprise ?':'Entreprise2','Dans quelle ville es-tu en alternance ?':'Ville2','Code postal de ton lieu de stage':'ZIP','Raconte nous ce que tu fais ou tes missions dans ce petit paragraphe suivant (pas obligatoire)':'Missions'
    # ...
}
...
df = dataset2[head1]
# Adding columns for x, y coordinates
cordX = pd.Series([0] * len(df))
cordY = pd.Series([0] * len(df))



# adding text columns
Ville0 = pd.Series(['NULL'] * len(df))
Entreprise0 = pd.Series(['NULL'] * len(df))

df['Ville0'] = Ville0
df['Entreprise0'] = Entreprise0

df['X'] = cordX
df['Y'] = cordY


#remplacer dans les nouvelles colonnes les valeurs non nulles
df['Ville0'] = df['Ville'].fillna(df['Ville2'])
df['Entreprise0'] = df['Entreprise1'].fillna(df['Entreprise2'])
# Vérifiez les valeurs de V0
...
gens = gpd.GeoDataFrame(datasetmap,
    geometry = gpd.points_from_xy(datasetmap['Y'], datasetmap['X']),
    crs = 'EPSG:4326').to_file('gens_point.shp')
